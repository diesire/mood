...
(function(global) {
    ...
    function normalizePkgDescriptor(descriptor) {
        ...
        path = descriptor.path = removeEndSlash(descriptor['path'] ||
            descriptor['location'] || '');
    ...
    function removeEndSlash(path) {
        return path && path.charAt(path.length - 1) == '/' ? path.substr(0,
            path.length - 1) : path;
    ...
    function Promise() {
        ...
        complete = function promiseComplete(success, arg) {
            ...
            then = success ? function(resolved, rejected) {
    ...
    core = {
        ...
        createResourceDef: function(id, cfg, isPreload, optCtxId) {
            ...
            function localRequire(ids, callback) {
                ...
                if (isType(ids, 'String')) {
                    ...
                    if (!(rvid in cache) || (isPromise(childDef) && !
                        earlyExport)) {
        ...
        getDefUrl: function(def) {
            ...
            return def.url || (def.url = core.checkToAddJsExt(def.require[
                'toUrl'](def.id)));
        },
        ...
        extractCfg: function(cfg) {
            ...
            cfg.pluginPath = 'pluginPath' in cfg ? cfg['pluginPath'] :
                'curl/plugin';
            ...
            function fixAndPushPaths(coll, isPkg) {
                ...
                for (var name in coll) {
                    ...
                    // property name.
                    parts = pluginParts(removeEndSlash(data['id'] || data[
                        'name'] || name));
                    id = parts.resourceId;
        ...
        resolvePathInfo: function(id, cfg, forPlugin) {
            ...
            if (forPlugin && cfg.pluginPath && id.indexOf('/') < 0 && !(id in
                pathMap)) {
        ...
        resolveUrl: function(path, cfg) {
            ...
            return baseUrl && !absUrlRx.test(path) ? joinPath(baseUrl, path) :
                path;
        },
        ...
        extractCjsDeps: function(defFunc) {
            ...
            source.replace(removeCommentsRx, '').replace(findRValueRequiresRx, function(
                m, id, qq) {
        ...
        fixArgs: function(args) {
            ...
            if (!deps && isDefFunc && defFunc.length > 0) {
                cjs = true;
                deps = ['require', 'exports', 'module'].concat(core.extractCjsDeps(
                    defFunc));
            }
        ...
        resolveResDef: function(def, args) {
            ...
            core.getDeps(def, args.deps, function(deps) {
                var resource;
                def.deps = deps;
                def.res = args.res;
                try {
        ...
        fetchResDef: function(def) {
            ...
            function() {
                ...
                if (def.useNet !== false) {
                    ...
                    if (!args || args.ex) {
                        def.reject(new Error(((args && args.ex) ||
                            'define() missing or duplicated: url').replace(
                            'url', def.url)));
                    } else {
        ...
        fetchDep: function(depName, parentDef) {
            ...
            def = cache[mainId];
            if (!def) {
                def = cache[mainId] = core.createResourceDef(mainId, pathInfo.config,
                    isPreload, pathInfo.path);
            ...
            if (mainId == loaderId) {
                ...
                when(def, function(plugin) {
                    ...
                    if (!normalizedDef) {
                        ...
                        normalizedDef = core.createResourceDef(fullId, resCfg,
                            isPreload, resId);
    ...
    cjsGetters = {
        'require': core.getCjsRequire,
        'exports': core.getCjsExports,
        'module': core.getCjsModule
    };
