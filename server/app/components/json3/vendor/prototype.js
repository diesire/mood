...
(function() {
    ...
    function toHTML(object) {
        return object && object.toHTML ? object.toHTML() : String.interpret(
            object);
    }
...
Object.extend(String.prototype, (function() {
    ...
    function truncate(length, truncation) {
        ...
        return this.length > length ?
            this.slice(0, length - truncation.length) + truncation : String(
            this);
    }
    ...
    function camelize() {
        ...
        for (var i = 1; i < len; i++)
            camelized += parts[i].charAt(0).toUpperCase() + parts[i].substring(
                1);
...
(function() {
    ...
    function uniq(sorted) {
        return this.inject([], function(array, value, index) {
            if (0 == index || (sorted ? array.last() != value : !array.include(
                value)))
                array.push(value);
            return array;
        });
...
var Hash = Class.create(Enumerable, (function() {
    function initialize(object) {
        this._object = Object.isHash(object) ? object.toObject() : Object.clone(
            object);
    }
        ...
        if (state == 'Complete') {
            try {
                ...
                (this.options['on' + response.status] || this.options['on' + (
                    this.success() ? 'Success' : 'Failure')] || Prototype.emptyFunction)(
            ...
            if (this.options.evalJS == 'force' || (this.options.evalJS && this.isSameOrigin() &&
...
Ajax.Response = Class.create({
    ...
    _getHeaderJSON: function() {
        ...
        try {
            return json.evalJSON(this.request.options.sanitizeJSON || !this.request
                .isSameOrigin());
        } catch (e) {
    ...
    _getResponseJSON: function() {
        ...
        if (!options.evalJSON || (options.evalJSON != 'force' && !(this.getHeader(
            'Content-type') || '').include('application/json')) ||
            this.responseText.blank())
            return null;
        try {
            return this.responseText.evalJSON(options.sanitizeJSON || !this.request
                .isSameOrigin());
        } catch (e) {
            this.request.dispatchException(e);
        }
...
(function(global) {
    ...
    var SETATTRIBUTE_IGNORES_NAME = (function() {
        ...
        var isBuggy = elForm.elements ? (typeof elForm.elements.test ==
            "undefined") : null;
    ...
    global.Element = function(tagName, attributes) {
        ...
        if (!cache[tagName]) cache[tagName] = Element.extend(document.createElement(
                tagName));
...
Element.Methods = {
    ...
    down: function(element, expression, index) {
        ...
        return Object.isNumber(expression) ? Element.descendants(element)[
            expression] :
    ...
    previous: function(element, expression, index) {
        element = $(element);
        if (arguments.length == 1) return $(Selector.handlers.previousElementSibling(
                element));
    ...
    next: function(element, expression, index) {
        element = $(element);
        if (arguments.length == 1) return $(Selector.handlers.nextElementSibling(
                element));
...
if (Prototype.Browser.Opera) {
    ...
    Element.Methods.getOffsetParent = Element.Methods.getOffsetParent.wrap(function(
        proceed, element) {
        element = $(element);
        try {
    ...
    Element.Methods.cumulativeOffset = Element.Methods.cumulativeOffset.wrap(function(
        proceed, element) {
        try {
            element.offsetParent
        } catch (e) {
    ...
    Element.Methods.getStyle = function(element, style) {
        ...
        if (style == 'opacity') {
            if (value = (element.getStyle('filter') || '').match(
                /alpha\(opacity=(.*)\)/))
        ...
        if (value == 'auto') {
            if ((style == 'width' || style == 'height') && (element.getStyle(
                'display') != 'none'))
    ...
    if (Prototype.BrowserFeatures.ElementExtensions) {
        (function() {
            ...
            Element.Methods.down = function(element, expression, index) {
                ...
                return Object.isNumber(expression) ? _descendants(element)[
                    expression] :
                    Element.select(element, expression)[index || 0];
            }
        })();
...
Object.extend(Selector, {
        ...
        pseudos: {
            ...
            'not': function(m) {
                ...
                while (e && le != e && (/\S/).test(e)) {
                    ...
                    for (var i = 0; i < len; i++) {
                        name = p[i].name
                        if (m = e.match(p[i].re)) {
                            v = Object.isFunction(x[name]) ? x[name](m) : new Template(
                                x[name]).evaluate(m);
            ...
            'nth-child': function(m) {
                return Selector.xpath.pseudos.nth(
                    "(count(./preceding-sibling::*) + 1) ", m);
            },
            'nth-last-child': function(m) {
                return Selector.xpath.pseudos.nth(
                    "(count(./following-sibling::*) + 1) ", m);
            },
    ...
    criteria: {
        ...
        attr: function(m) {
            m[3] = (m[5] || m[6]);
            return new Template(
                'n = h.attr(n, r, "#{1}", "#{3}", "#{2}", c); c = false;').evaluate(
                m);
        },
        pseudo: function(m) {
            if (m[6]) m[6] = m[6].replace(/"/g, '\\"');
            return new Template(
                'n = h.pseudo(n, "#{1}", "#{6}", r, c); c = false;').evaluate(m);
        },
    ...
    handlers: {
        ...
        index: function(parentNode, reverse, ofType) {
            ...
            if (reverse) {
                for (var nodes = parentNode.childNodes, i = nodes.length - 1, j =
                        1; i >= 0; i--) {
                    var node = nodes[i];
        ...
        child: function(nodes) {
            ...
            for (var i = 0, results = [], node; node = nodes[i]; i++) {
                for (var j = 0, child; child = node.childNodes[j]; j++)
                    if (child.nodeType == 1 && child.tagName != '!') results.push(
                            child);
            }
            return results;
        },
        ...
        tagName: function(nodes, root, tagName, combinator) {
            ...
            if (nodes) {
                ...
                for (var i = 0, node; node = nodes[i]; i++)
                    if (node.tagName.toUpperCase() === uTagName) results.push(
                            node);
                return results;
        ...
        id: function(nodes, root, id, combinator) {
            ...
            if (nodes) {
                if (combinator) {
                    if (combinator == 'child') {
                        for (var i = 0, node; node = nodes[i]; i++)
                            if (targetNode.parentNode == node) return [
                                    targetNode];
                        ...
                        for (var i = 0, node; node = nodes[i]; i++)
                            if (Selector.handlers.previousElementSibling(
                                targetNode) == node)
                                return [targetNode];
        ...
        byClassName: function(nodes, root, className) {
            ...
            for (var i = 0, results = [], node, nodeClassName; node = nodes[i]; i++) {
                ...
                if (nodeClassName == className || (' ' + nodeClassName + ' ').include(
                    needle))
                    results.push(node);
            }
            return results;
    ...
    pseudos: {
        ...
        'only-child': function(nodes, value, root) {
            var h = Selector.handlers;
            for (var i = 0, results = [], node; node = nodes[i]; i++)
                if (!h.previousElementSibling(node) && !h.nextElementSibling(
                    node))
                    results.push(node);
            return results;
        },
        ...
        var data = elements.inject({}, function(result, element) {
            if (!element.disabled && element.name) {
                ...
                if (value != null && element.type != 'file' && (element.type !=
                    ...
                    submitted = true)))) {
                    if (key in result) {
                        if (!Object.isArray(result[key])) result[key] = [result[
                                    key]];
...
(function() {
    ...
    function element(event) {
        ...
        if (currentTarget && currentTarget.tagName) {
            if (type === 'load' || type === 'error' ||
                ...
                'input' && currentTarget.type === 'radio'))
                node = currentTarget;
        }
    ...
    if (Prototype.Browser.IE) {
        ...
        Event.prototype = window.Event.prototype || document.createEvent(
