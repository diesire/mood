...
var path = require("path"),
    ...
    closureOptions = ["--compilation_level=ADVANCED_OPTIMIZATIONS",
            "--warning_level=QUIET"
    ];
...
fs.readFile(path.join(__dirname, "README.md"), "utf8", function readInfo(
    exception, source) {
    if (exception) {
        console.log(exception);
    } else {
        // Read the project page template.
        fs.readFile(path.join(__dirname, "page", "page.html"), "utf8",
            readTemplate);
    }
    ...
    function readTemplate(exception, page) {
        ...
        if (exception) {
            ...
            headers.forEach(function(value) {
                ...
                lines[index] = Array(level + 1).join("#") +
                    "<a name=\"section_" + section.join(".") + "\"></a>" + text;
                value.push(section);
                lastSection = section;
                lastLevel = level;
            });
            ...
            navigation = headers.map(function(value) {
                var index = value[0],
                    ...
                    section = value[3],
                    name = section.join(".");
                return "<li><a href=\"#section_" + name + "\">" + text +
                    "</a></li>";
            });
            navigation.push("");
            // Write the page source to disk.
            fs.writeFile(path.join(__dirname, "index.html"), page.replace(
                /<%=\s*(.+?)\s*%>/g, function interpolate(match, data) {
                switch (data) {
                    case "navigation":
                ...
                console.log(exception ||
                    "GitHub project page generated successfully.");
            });
        }
    }
});
...
fs.readFile(path.join(__dirname, "lib", "json3.js"), "utf8", function readSource(
    exception, source) {
    ...
    if (exception) {
        ...
        compiler.stdin.end(source.replace(
            /^;?\(function\s*\(\)\s*\{([\s\S]*?)}\)\.call\(this\);*?/m, "$1"));
    }
    ...
    function compressSource(exception, compressed) {
        if (exception) {
            ...
            fs.writeFile(path.join(__dirname, "lib", "json3.min.js"),
                compressed, writeSource);
        }
        ...
        function writeSource(exception) {
            console.log(exception ||
                "Compressed version generated successfully.");
            ...
            case "/":
                ...
                switch (symbol) {
                    ...
                    case "/":
                        ...
                        results.push(original + source.slice(index, index =
