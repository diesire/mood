...
angular.mock.$Browser = function() {
    ...
    self.defer.flush = function(delay) {
        ...
        while (self.deferredFns.length && self.deferredFns[0].time <= self.defer
            .now) {
...
angular.mock.$LogProvider = function() {
    ...
    this.$get = function() {
        ...
        $log.assertEmpty = function() {
            ...
            angular.forEach(['error', 'warn', 'info', 'log'], function(logLevel) {
                angular.forEach($log[logLevel].logs, function(log) {
                    angular.forEach(log, function(logItem) {
                        errors.push('MOCK $log (' + logLevel + '): ' + String(
                            logItem) + '\n' + (logItem.stack || ''));
                    });
                });
            });
            if (errors.length) {
                errors.unshift(
                    "Expected $log to be empty! Either a message was logged unexpectedly, or an expected " +
        ...
        if (expectation && expectation.match(method, url)) {
            ...
            if (expectation.response) {
                responses.push(function() {
                    var response = expectation.response(method, url, data,
                        headers);
            ...
            if (definition.match(method, url, data, headers || {})) {
                if (definition.response) {
                    ...
                    ($browser ? $browser.defer : responsesPush)(function() {
                        var response = definition.response(method, url, data,
                            headers);
                        xhr.$$respHeaders = response[2];
                        callback(response[0], response[1], xhr.getAllResponseHeaders());
                    });
...
function MockXhr() {
    ...
    this.getResponseHeader = function(name) {
        ...
        angular.forEach(this.$$respHeaders, function(headerVal, headerName) {
            if (!header && angular.lowercase(headerName) == name) header =
                    headerVal;
        });
        return header;
    };
